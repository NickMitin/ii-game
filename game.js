...
for (let i = 0; i < map.length; ++i) {
  ...
  characterStepsprite.style.backgroundPositionX = '-' + (characterStepspriteImage[0] * tileSize) + 'px'
  characterStepsprite.style.backgroundPositionY = '-' + (characterStepspriteImage[1] * tileSize) + 'px'
...
setInterval(() => {
  if (currentState === 'jumping') {
    ...
    if (tileCode === 'h' || tileCode === 'x' || tileCode === '^' || tileCode === '>' || tileCode === '<' || tileCode === 'g' || tileCode === 'm' || tileCode === 'v') {
      playscream()
      characterTileX++;
      characterScale -= 0.3
      if (characterTileX > 3) {
...
function showChartherTile(tileX, tileY) {
  characterSprite.style.backgroundPositionX = '-' + (tileX * charactetileSize) + 'px'
  characterSprite.style.backgroundPositionY = '-' + (tileY * charactetileSize) + 'px'
  characterSprite.style.transform = 'scale(' + characterScale + ')'
}
